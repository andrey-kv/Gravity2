...
public class Figure {
	...
	Figure(DrawPanel drawPanel) {
		...
		this.color = DrawPanel.FIG_COLOR[getRandom(0,DrawPanel.FIG_COLOR.length)];
	...
	public void draw(Graphics2D g2) {
		...
		int currentX = currentPoint.x - (figureSize / 2);
		int currentY = currentPoint.y - (figureSize / 2);
		g2.setColor(color);
		Shape fg = new Ellipse2D.Double(currentX, currentY, figureSize, figureSize);
		g2.draw(fg);
	}
	...
	public void checkPosition(Figure[] figures) {
		...
		mechanics.calculateNextCoord();
		mechanics.calculateNextSpeed();
		if (mechanics.nextSpeed.getValue()==0) return;
	...
	private void reflectWithA(Figure fig) {
		if (getLocation().distance(fig.getLocation())<figureSize/2+fig.figureSize/2) {
			...
			double relative = Math.cos(relativeDistanceAngle - relativeSpeedAngle);
			if (relative > 0) {
	...
	// ¬озвращает случайное целое в промежутке от min до max.
